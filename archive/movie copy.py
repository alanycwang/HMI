...
class Movie(FigureCanvasQTAgg):
    ...
    def __init__(self, maps, player, clim=1000, w=640, h=480, cea=False, parent=None, **kwargs):
        ...
        fig, _, im = self.maps[0].plot(clim=self.clim, cea=self.cea, scale=self.scale, crop=self.crop, frame=self.crop_frame, rot=self.rotate, start_time=self.crop_time, skip_reset=True, type=self.type)
    ...
    def redraw(self):
        ...
        for i in range(len(self.maps)):
            fig, _, im = self.maps[i].plot(clim=self.clim, cea=self.cea, scale=self.scale, crop=self.crop, frame=self.crop_frame, rot=self.rotate, start_time=self.crop_time, type=self.type)
            fig.set_size_inches(self.fig_width, self.fig_height)
    ...
    def draw_specific(self, i):
        fig, _, im = self.maps[i].plot(clim=self.clim, cea=self.cea, scale=self.scale, crop=self.crop, frame=self.crop_frame, type=self.type)
        fig.set_size_inches(self.fig_width, self.fig_height)
        self.figs[i] = fig
        fig.canvas.draw()
        self.im[i] = im
    ...
    def release_zoom(self, event):
        ...
        if self.type[-6:] == 'zoomed':
            self.crop=True
            fig, _, im = self.maps[self.player.i].plot(clim=self.clim, cea=self.cea, scale=self.scale, crop=self.crop, frame=self.crop_frame, type=self.type)
            fig.set_size_inches(self.fig_width, self.fig_height)
            self.figure = fig
            self.real = True
            self.connect_events()
    ...
    def home(self):
        ...
        fig, _, im = self.maps[self.player.i].plot(clim=self.clim, cea=self.cea, scale=self.scale, crop=self.crop,
                                            frame=self.crop_frame, type=self.type)
        fig.set_size_inches(self.fig_width, self.fig_height)
        self.figure = fig
        self.real = True
        self.connect_events()
